...
It currently contains three linked literature workflows plus one utility skill:
...
4. `codex-claude-skill-bootstrap`: build safe one-command installers or migration packages for user-owned Codex/Claude skills.
...
Builds safe installation or migration packages for user-owned Codex and Claude skills. It is intended for tasks like:

- packaging user-owned skills into a public or private GitHub repository
- creating `install.sh` and `installer/skills-manifest.json`
- installing selected skills into `~/.codex/skills` and/or `~/.claude/skills`
- detecting official or proprietary skills without copying their contents into a public repository
- avoiding credentials, browser tokens, session URLs, and restricted third-party content in installers
...
### `skills/codex-claude-skill-bootstrap`

The utility skill for building installers and migration packages for user-owned skills. It deliberately treats Claude/Codex official skills and proprietary skills as detect-only items for public repositories, so a public installer can stay redistributable.
...
Install the Codex/Claude skill bootstrap utility:

```bash
mkdir -p ~/.codex/skills/codex-claude-skill-bootstrap
cp -R read_paper_skills/skills/codex-claude-skill-bootstrap/* \
      ~/.codex/skills/codex-claude-skill-bootstrap/
```
...
To create an installer or migration package for user-owned skills:

```text
[$codex-claude-skill-bootstrap] 请帮我为这些自有 Codex/Claude skills
创建一个安全的一键安装包。公开仓库只放自有 skills；
官方或 Proprietary skills 只检测，不提交内容。
```
